// app/api/timeseries/analyze/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📤 Sending timeseries analysis request...');
    console.log('📋 Request params:', request.nextUrl.searchParams.toString());
    
    const response = await fetch(`${fastApiUrl}/api/timeseries/analyze?${request.nextUrl.searchParams.toString()}`, {
      method: 'POST',
      body: formData,
    });
    ...
    try {
      ...
      // フォールバック処理
      if (responseText.includes('=== 時系列分析API処理完了 ===') || 
          responseText.includes('時系列分析が完了しました') || 
          response.status === 200) {
    ...
    console.log('✅ Timeseries analysis completed successfully');
    ...
    console.error('❌ Timeseries analysis error:', error);
