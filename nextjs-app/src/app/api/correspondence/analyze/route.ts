// app/api/correspondence/analyze/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📤 Sending correspondence analysis request...');
    
    const response = await fetch(`${fastApiUrl}/api/correspondence/analyze?${request.nextUrl.searchParams.toString()}`, {
      method: 'POST',
      body: formData,
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: 'コレスポンデンス分析に失敗しました', details: errorText },
    ...
    console.log('✅ Correspondence analysis completed successfully');
