// app/api/cluster/analyze/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 本番環境では環境変数で適切なURLを設定
    const fastApiUrl = process.env.FASTAPI_URL || 'http://python-api:8000';
    
    console.log('🔗 FastAPI URL:', fastApiUrl);
    console.log('📤 Sending cluster analysis request...');
    
    const response = await fetch(`${fastApiUrl}/api/cluster/analyze?${request.nextUrl.searchParams.toString()}`, {
      method: 'POST',
      body: formData,
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: 'クラスター分析に失敗しました', details: errorText },
    ...
    try {
      ...
      if (responseText.includes('=== クラスター分析API処理完了 ===') || response.status === 200) {
